...
def get_all_urls(request):
    user_timezone = request.GET.get('timezone', 'UTC')  # Default to UTC if not provided
    ...
    try:
        ...
        return Response({"error": "Unknown timezone."}, status=400)

    current_time_user = current_time_utc.astimezone(user_tz)

    # Filter URLs that are not expired
    urls = URL.objects.filter(expires_at__isnull=True) | URL.objects.filter(expires_at__gt=current_time_user)
